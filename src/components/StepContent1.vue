...
import cpService from "../api/komponen/cpService";
import uvpyService from "../api/komponen/uvpyService";
import uvppService from "../api/komponen/uvppService";
...
const upload = async () => {
  ...
  //   return;
  // }
  if (!dropdownKomponen.value) {
    props.toast.add({
      severity: "warn",
      summary: "Warning Message",
      detail: "Pilih komponen dan data terlebih dahulu",
      life: 3000,
    });
    return;
  }
  ...
  if (props.templateName === "CP")
    dataPreview.value = await cpService.getCps(dropdownKomponen.value.code);
  else if (props.templateName === "PY")
    dataPreview.value = await uvpyService.getUvpys(dropdownKomponen.value.code);
  else if (props.templateName === "PP")
    dataPreview.value = await uvppService.getUvpps(dropdownKomponen.value.code);
...
<template>
  <div class="flex col-span-full lg:col-span-6">
    <div class="card px-0!">
      <Select
        v-model="dropdownKomponen"
        :options="dropdownKomponens"
        optionLabel="name"
  ...
  <div v-if="dataPreview.length === 0" class="flex flex-col h-48">
    <div
